    ...
    LEFT JOIN legacy.v_bag_nag_bag_ore_ligt_aan_openbareruimte rel_0
    ...
    LEFT JOIN legacy.v_bag_ore_bag_wps_ligt_in_woonplaats rel_1
    ...
    LEFT JOIN (
          ...
          FROM (
                   SELECT *
                   FROM legacy.v_bag_vot_bag_nag_heeft_hoofdadres
                   UNION
                   SELECT *
                   FROM legacy.v_bag_vot_bag_nag_heeft_nevenadres
    ...
    LEFT JOIN legacy.g_verblijfsobjecten adresseert_verblijfsobject ON vot_adressen.src_id = adresseert_verblijfsobject._id and vot_adressen.src_volgnummer = adresseert_verblijfsobject.volgnummer
    ...
    LEFT JOIN (
          ...
          FROM (
                   SELECT *
                   FROM legacy.v_bag_lps_bag_nag_heeft_hoofdadres
                   UNION
                   SELECT *
                   FROM legacy.v_bag_lps_bag_nag_heeft_nevenadres
               ) q GROUP BY dst_id, dst_volgnummer, src_id
      ) lps_adressen ON lps_adressen.dst_id = nag._id AND lps_adressen.dst_volgnummer = nag.volgnummer
    LEFT JOIN legacy.g_ligplaatsen adresseert_ligplaats ON lps_adressen.src_id = adresseert_ligplaats._id and lps_adressen.src_volgnummer = adresseert_ligplaats.volgnummer
    ...
    LEFT JOIN (
          ...
          FROM (
                   SELECT *
                   FROM legacy.v_bag_sps_bag_nag_heeft_hoofdadres
                   UNION
                   SELECT *
                   FROM legacy.v_bag_sps_bag_nag_heeft_nevenadres
               ) q GROUP BY dst_id, dst_volgnummer, src_id
      ) sps_adressen ON sps_adressen.dst_id = nag._id AND sps_adressen.dst_volgnummer = nag.volgnummer
    LEFT JOIN legacy.g_standplaatsen adresseert_standplaats ON sps_adressen.src_id = adresseert_standplaats._id and sps_adressen.src_volgnummer = adresseert_standplaats.volgnummer
    -- SELECT in_onderzoek
    LEFT JOIN (
          SELECT
              src_id, src_volgnummer
          FROM legacy.v_bag_nag_bag_ozk_heeft_onderzoeken rel
