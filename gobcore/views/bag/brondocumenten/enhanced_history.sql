...
FROM (
  ...
  INNER JOIN legacy.g_ligplaatsen lig
  ...
  INNER JOIN legacy.g_nummeraanduidingen nag
  ...
  INNER JOIN legacy.g_openbareruimtes ore
  ...
  INNER JOIN legacy.g_panden pnd
  ...
  INNER JOIN legacy.g_standplaatsen sps
  ...
  INNER JOIN legacy.g_verblijfsobjecten vot
  ...
  INNER JOIN legacy.g_openbareruimtes wps
...
LEFT JOIN (
  ...
  INNER JOIN legacy.v_bag_dsr_bag_bdt_heeft_brondocumenten dsr_bdt
    ON dsr."_source_id" = dsr_bdt.src_id
    AND dsr._date_deleted IS NULL
) dsr ON dsr.dossier = bag.dossier
LEFT JOIN legacy.g_brondocumenten bdt ON dsr.heeft_brondocument = bdt.documentnummer
