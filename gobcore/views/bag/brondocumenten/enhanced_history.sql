...
FROM (
  ...
  FROM legacy.mv_bag_lps_bag_dsr_heeft_dossier lps_dsr
  INNER JOIN legacy.bag_ligplaatsen lig
  ...
  FROM legacy.mv_bag_nag_bag_dsr_heeft_dossier nag_dsr
  INNER JOIN legacy.bag_nummeraanduidingen nag
  ...
  FROM legacy.mv_bag_ore_bag_dsr_heeft_dossier ore_dsr
  INNER JOIN legacy.bag_openbareruimtes ore
  ...
  FROM legacy.mv_bag_pnd_bag_dsr_heeft_dossier pnd_dsr
  INNER JOIN legacy.bag_panden pnd
  ...
  FROM legacy.mv_bag_sps_bag_dsr_heeft_dossier sps_dsr
  INNER JOIN legacy.bag_standplaatsen sps
  ...
  FROM legacy.mv_bag_vot_bag_dsr_heeft_dossier vot_dsr
  INNER JOIN legacy.bag_verblijfsobjecten vot
  ...
  FROM legacy.mv_bag_wps_bag_dsr_heeft_dossier wps_dsr
  INNER JOIN legacy.bag_openbareruimtes wps
...
LEFT JOIN (
  ...
  FROM legacy.bag_dossiers dsr
  INNER JOIN legacy.mv_bag_dsr_bag_bdt_heeft_brondocumenten dsr_bdt
    ON dsr."_source_id" = dsr_bdt.src_id
    AND dsr._date_deleted IS NULL
) dsr ON dsr.dossier = bag.dossier
LEFT JOIN legacy.bag_brondocumenten bdt ON dsr.heeft_brondocument = bdt.documentnummer
