SELECT
  bpg.identificatie,
    ...
    kot.indexletter,
    kot.indexnummer
FROM legacy.wkpb_beperkingen bpg
INNER JOIN legacy.rel_wkpb_bpg_brk_kot_belast_kadastrale_objecten rel_bpg_kot
  ON rel_bpg_kot.src_id = bpg._id
INNER JOIN legacy.brk_kadastraleobjecten kot
  ON  rel_bpg_kot.dst_id = kot._id
  AND rel_bpg_kot.dst_volgnummer = kot.volgnummer
INNER JOIN legacy.mv_brk_kot_brk_kse__angdd_dr__kadastralesectie rel_kot_kse
  ON kot."_id"  = rel_kot_kse.src_id
  AND kot.volgnummer  = rel_kot_kse.src_volgnummer
INNER JOIN legacy.brk_kadastralesecties kse
